...
#			surv == survival: 1 == yes, = == no
#			flow == flowering: 1 == yes, = == no
...
tmp=order(site[ros==1]) ## Create array used to order next handful

size.t.all=log(nl.t*ll.t)[ros==1][tmp] ## plant sizes in year t
size.t1.all=log(nl.t1*ll.t1)[ros==1][tmp] ## plant sizes in year t+1

flow.all=flow[ros==1][tmp] ## Logical vector if flowering: 1 = yes
table(flow.all)
surv.all=surv[ros==1][tmp] ## Logical vector of survival: 1 = yes

site.all=site[ros==1][tmp] ## Site vector
...
size.t=size.t.all[flow.all==0] ## size of plants that didn't flower in year t
size.t1=size.t1.all[flow.all==0] ## Size of plants that didn't flower in year t+1
site.s=site.all[flow.all==0] ## ??

test=complete.cases(size.t,size.t1,site.s) ## Logical vector of complete cases

size.t=size.t[test] ## Filter for complete cases
...
summary(fit.grow.gls)
intervals(fit.grow.gls) ## What is this?

g.intercepts=fit.grow.gls$coef[1:2] ## Growth intercepts
g.slopes=rep(fit.grow.gls$coef[3],2) ## Growth slopes
...
flow.s=flow.all[order(size.t.all)]; length(flow.s) ## flower logical vector in order of size.t.all
site.s=site.all[order(size.t.all)]; length(site.s) ## site vector in order of size.t.all
size.t=size.t.all[order(size.t.all)]; length(size.t) ## size vector in order of size.t.all
...
surv.s=surv.all[order(size.t.all)] ## Survival B vector in order of size.t.all
site.s=site.all[order(size.t.all)] ## site vector in order of size.t.all
size.t=size.t.all[order(size.t.all)] ## size vector in order of size.t.all
...
plot(size.t.f,log(si.t1.f),
     type="n",
     xlab="Plant size year t",
     ylab="Viable seeds (log scale) year t+1",
     pch=19,
     xlim=c(2,10),
     ylim=c(2,10))
...
seedlings.size.t=IPM.seedlings$size.t[IPM.seedlings$site!="JU"] ## Seedling size in two main sites year t

seedlings.site=IPM.seedlings$site[IPM.seedlings$site!="JU"] ## seedling sites

tmp=order(seedlings.site) ## Another vector for order

seedlings.size.t=seedlings.size.t[tmp] ## Order seedlings according to tmp order vector

seedlings.site=seedlings.site[tmp] ## Order seedlings according to tmp order vector

fit.seedlings=lm(seedlings.size.t~seedlings.site-1) ## That -1! What is that?
...
## Order vector from establishment data
tmp=order(IPM.establishment$site)

p.est.site=IPM.establishment$site[tmp] ## Site of est. obs.
p.est.seeds.t=IPM.establishment$seeds.t[tmp] ## Seed production in plot in year t
p.est.seedlings=IPM.establishment$sdl.t1[tmp] ## Seedlings in plot in year t+1

## Seedlings per site/seeds per site for establishment rate
...
## Empty vector for values. This could be cleaned up with a nested for loop
p.vec<-rep(0,12)
...
#===============================================================
#	Calculation: Evolutionary Stable Strategy
#===============================================================
...
#===============================================================
#	Plot: Evolutionary Stable Strategy (see Fig. 6)
#===============================================================
