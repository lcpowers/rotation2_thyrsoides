...
# dataf$site = as.factor(dataf$site)

tmp = order(site[ros==1])

size.t.all=log(nl.t*ll.t)[ros==1][tmp]
size.t1.all=log(nl.t1*ll.t1)[ros==1][tmp]

flow.all=flow[ros==1][tmp]

surv.all=surv[ros==1][tmp]

site.all=site[ros==1][tmp]
...
fit.grow.gls.1<-gls(size.t1~size.t+site.s,na.action=na.omit,weight=varExp(form=~fitted(.)|site.s),method="ML");
summary(fit.grow.gls.1)

fit.grow.gls<-gls(size.t1~size.t+site.s,na.action=na.omit,weight=varExp(form=~fitted(.)),method="ML");
summary(fit.grow.gls)

fit.grow.gls.0<-gls(size.t1~size.t+site.s,na.action=na.omit,method="ML");
...
fit.grow.gls.0<-gls(size.t1~size.t,na.action=na.omit,weight=varExp(form=~fitted(.)|site.s),method="ML"); # Generalized least squares, maximum likelihood
fit.grow.gls.1<-gls(size.t1~size.t+site.s,na.action=na.omit,weight=varExp(form=~fitted(.)|site.s),method="ML");
fit.grow.gls.2<-gls(size.t1~size.t*site.s,na.action=na.omit,weight=varExp(form=~fitted(.)|site.s),method="ML");
...
fit.grow.gls<-gls(size.t1~site.s+size.t-1,na.action=na.omit,weight=varExp(form=~fitted(.)|site.s),method="ML");
