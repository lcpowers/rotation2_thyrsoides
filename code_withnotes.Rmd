...
while(any(search()=="dataf")) detach(dataf)
while(any(search()=="growth_df")) detach(growth_df)
```
...
## Increase growth rates -- size.t1 should be a little higher for faster growth rate
```{r growth}

sites = c("FU","SP")

inc_size.t1 <- rep(-9999,nrow(growth_df))
dec_size.t1 <- rep(-9999,nrow(growth_df))

for(site_i in sites){
  
  size_diffs <- growth_df$size.t1[growth_df$site.s == site_i] - growth_df$size.t[growth_df$site.s == site_i]
  
  # Look at distributions
  par(mfrow = c(1,1))
  hist(size_diffs, breaks=20, main = site_i)
  
  # Set changes
  avg_size_diffs <- mean(size_diffs,na.rm = T)
  sd_size_diffs <- sd(size_diffs,na.rm = T)
  
  size_diff_changes <- rnorm(n = length(size_diffs),
                                mean = avg_size_diffs,
                                sd = sd_size_diffs)
  
  growth_df$inc_size.t1[growth_df$site.s == site_i] <- growth_df$size.t1[growth_df$site.s == site_i] + size_diff_changes

  # Resample to create vector to decrease year t + 1 sizes by
  size_diff_changes <- rnorm(n = length(size_diffs), 
                             mean = avg_size_diffs, 
                             sd = sd_size_diffs)

  # New column with smaller year t1 sizes
  growth_df$dec_size.t1[growth_df$site.s == site_i] <- growth_df$size.t1[growth_df$site.s == site_i] - size_diff_changes
  
  # Compare distributions of true sizes to fake larger sizes
  par(mfrow = c(1,3))
  hist(growth_df$size.t1[growth_df$site.s == site_i], breaks=20, main = site_i, xlab = "no change")
  hist(growth_df$inc_size.t1[growth_df$site.s == site_i], breaks=20, main = site_i, xlab = "size inc")
  hist(growth_df$dec_size.t1[growth_df$site.s == site_i], breaks=20, main = site_i, xlab = "size dec")
  
  # Compare summaries of true sizes to fake smaller sizes
  print(c(site_i,"no change:"))
  print(summary(growth_df$size.t1[growth_df$site.s == site_i]))
  cat("\n")
  print(c(site_i,"increase:"))
  print(summary(growth_df$inc_size.t1[growth_df$site.s == site_i]))
  cat("\n")
  print(c(site_i,"decrease:"))
  print(summary(growth_df$dec_size.t1[growth_df$site.s == site_i]))
  cat("----\n")
...
#### Refit growth equation using new year t1 data
```{r new_growth_fit}
# none, increase, or decrease
...
if(change_type == "decrease"){
  ...
  fit.grow.gls<-gls(dec_size.t1~site.s+size.t-1, ## Why -1?
                    na.action=na.omit,
                    weight=varExp(form=~fitted(.)|site.s),
                    method="ML",
                    data=growth_df)
...
if(change_type == "increase"){
  ...
  fit.grow.gls<-gls(inc_size.t1~site.s+size.t-1, ## Why -1?
                    na.action=na.omit,
                    weight=varExp(form=~fitted(.)|site.s),
                    method="ML",
                    data=growth_df)
...
#### #
#### Run all chunks abouve for change in growth
#### #
...
stable.dist=array(NA,dim=c(n,n.age,2)) ## 250x50x2 array -- 3rd dimension is site
lam.stable.age=rep(NA,2); ## 1x2 array for lambda at stable age distribution
...
plot(Res.p.est[,1],Res.p.est[,7],type="n",xlab=expression(paste(italic(p)[e])),ylab=expression(italic(T)),ylim=c(8,50))
